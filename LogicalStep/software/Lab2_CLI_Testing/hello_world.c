        ...
        case 'f':              /* FatFS API controls */
            ...
            case 'p':          /* fp <len> - read and play file from current fp */
                ...
                while(p1 > 0){
                    ...
                    for(index = 0; index < cnt; index += speed){
                    	while(alt_up_audio_write_fifo_space(audio_dev, ALT_UP_AUDIO_RIGHT) < speed &&
							alt_up_audio_write_fifo_space(audio_dev, ALT_UP_AUDIO_LEFT) < speed
						){}

                    	l_buf = (Buff[index+3] << 8) | (Buff[index+2] & 0xF);
                    	r_buf = (Buff[index+1] << 8) | (Buff[index] & 0xF);
